...
class DCVC_net(nn.Module):
    ...
    def motioncompensation(self, ref, mv):
        ...
        predict_frame = flow_warp(ref, mv)
        context = self.context_refine(prediction_init)

        return context, predict_frame

    def mv_refine(self, ref, mv):
        return self.mvDecoder_part2(torch.cat((mv, ref), 1)) + mv

    def quantize(self, inputs, mode, means=None, STEQuant=None):
        assert(mode == "dequantize")
        outputs = inputs.clone()
        outputs -= means
        if STEQuant is not None:
            outputs = self.STE_Quant(outputs)
        else:
            outputs = torch.round(outputs)
        outputs += means
        return outputs

    def feature_probs_based_sigma(self, feature, mean, sigma, mode, iter, total_iter, delta=1.0, STEQuant=None, finetune=None):
        if STEQuant is not None:
            outputs = self.STE_Quant(feature/delta)*delta
        else:
            if mode == "test":
                outputs = torch.round((feature - mean) / delta) * delta + mean
            elif mode != "test" and finetune is None:
                outputs = self.quant(feature, mode)
            else:
                outputs = self.Quantizator_SGA(feature/delta, iter, mode, total_iter)*delta

        values = outputs - mean
        mu = torch.zeros_like(sigma)
        sigma = sigma.clamp(1e-5, 1e10)
        gaussian = torch.distributions.laplace.Laplace(mu, sigma)
        probs = gaussian.cdf(values + delta/2) - gaussian.cdf(values - delta/2)
        total_bits = torch.sum(torch.clamp(-1.0 * torch.log(probs + 1e-5) / math.log(2.0), 0, 50))
        return total_bits, probs, delta
    ...
    def quant(self, x, mode, delta=1.0):
        if mode == "training":
    ...
    def forward(self, referframe, input_image, mode, stage, iter, total_iter, delta_mv=1.0, delta_res=1.0,
                mvfeature=None, z_mv=None, feature=None, z=None, force_sga=False):
        if stage == "ME_MC":
            estmv = self.opticFlow(input_image, referframe)

            mvfeature = self.mvEncoder(estmv)
            z_mv = self.mvpriorEncoder(mvfeature)
            compressed_z_mv = self.quant(z_mv, mode)
            params_mv = self.mvpriorDecoder(compressed_z_mv)
            quant_mv = self.quant(mvfeature, mode)
            ...
            quant_mv_upsample_refine = self.mv_refine(referframe, quant_mv_upsample)
            context, predict_frame = self.motioncompensation(referframe, quant_mv_upsample_refine)

            total_bits_mv, _ = self.feature_probs_based_sigma(mvfeature, means_hat_mv, scales_hat_mv, mode)
            ...
            bpp = bpp_mv_y + bpp_mv_z

            return {"bpp_mv_y": bpp_mv_y,
                    "bpp_mv_z": bpp_mv_z,
                    "bpp": bpp,
                    "recon_image": predict_frame,
                    "context": context,
                    }

        elif stage == "Finetuning_flow":

            compressed_z_mv = self.Quantizator_SGA(z_mv, iter, mode, total_iter)

            params_mv = self.mvpriorDecoder(compressed_z_mv)

            quant_mv = self.Quantizator_SGA(mvfeature / delta_mv, it=iter, mode=mode, total_it=total_iter) * delta_mv
            ...
            compressed_z = self.quant(z, mode)

            params = self.priorDecoder(compressed_z)

            feature_renorm = feature

            compressed_y_renorm = self.quant(feature_renorm, mode)
            ...
            total_bits_y, _, _ = self.feature_probs_based_sigma(
                feature_renorm, means_hat, scales_hat, mode, iter, total_iter, delta=1.0, STEQuant=None, finetune=None)
            total_bits_mv, _, _ = self.feature_probs_based_sigma(
                mvfeature, means_hat_mv, scales_hat_mv, mode, iter, total_iter, delta=delta_mv, STEQuant=None, finetune=1)
            ...
            compressed_z = self.Quantizator_SGA(z, iter, mode, total_iter)

            params = self.priorDecoder(compressed_z)

            feature_renorm = feature

            compressed_y_renorm = self.Quantizator_SGA(feature_renorm/delta_res, iter, mode, total_iter)*delta_res
            ...
            total_bits_y, _, _ = self.feature_probs_based_sigma(
                feature_renorm, means_hat, scales_hat, mode, iter, total_iter, delta=delta_res, STEQuant=None, finetune=1)
            total_bits_mv, _, _ = self.feature_probs_based_sigma(
                mvfeature, means_hat_mv, scales_hat_mv, mode, iter, total_iter, delta=1.0, STEQuant=None, finetune=None)
            total_bits_z, _ = self.iclr18_estrate_bits_z(compressed_z)
            total_bits_z_mv, _ = self.iclr18_estrate_bits_z_mv(compressed_z_mv)
        ...
        elif stage == "Finetuning_enc":
            estmv = self.opticFlow(input_image, referframe)

            mvfeature = self.mvEncoder(estmv)
            z_mv = self.mvpriorEncoder(mvfeature)
            compressed_z_mv = self.quant(z_mv, mode=mode, delta=1.0)

            params_mv = self.mvpriorDecoder(compressed_z_mv)

            quant_mv = self.quant(mvfeature, mode=mode, delta=1.0)
            ...
            compressed_z = self.quant(z, mode=mode, delta=1.0)

            params = self.priorDecoder(compressed_z)

            feature_renorm = feature

            compressed_y_renorm = self.quant(feature_renorm, mode=mode, delta=1.0)
            ...
            total_bits_y, _, _ = self.feature_probs_based_sigma(
                feature_renorm, means_hat, scales_hat, mode, iter, total_iter, delta=delta_res, STEQuant=None, finetune=None)
            total_bits_mv, _, _ = self.feature_probs_based_sigma(
                mvfeature, means_hat_mv, scales_hat_mv, mode, iter, total_iter, delta=1.0, STEQuant=None, finetune=None)
            total_bits_z, _ = self.iclr18_estrate_bits_z(compressed_z)
            total_bits_z_mv, _ = self.iclr18_estrate_bits_z_mv(compressed_z_mv)
            ...
            if force_sga:
                quant_mv = self.Quantizator_SGA(mvfeature / delta_mv, it=iter, mode="training", total_it=total_iter) * delta_mv
            else:
            ...
            total_bits_y, _, delta_res = self.feature_probs_based_sigma(
                feature_renorm, means_hat, scales_hat, mode, 0, total_iter, delta=1.0, STEQuant=None, finetune=None)
            total_bits_mv, _, delta_mv = self.feature_probs_based_sigma(
                mvfeature, means_hat_mv, scales_hat_mv, mode, 0, total_iter, delta=1.0, STEQuant=None, finetune=None)
            total_bits_z, _ = self.iclr18_estrate_bits_z(compressed_z)
            total_bits_z_mv, _ = self.iclr18_estrate_bits_z_mv(compressed_z_mv)
            ...
            return {"bpp_mv_y": bpp_mv_y,
                    ...
                    "feature": feature,
                    "z": z,
                    "delta_mv": torch.tensor(delta_mv).cuda(),
                    "delta_res": torch.tensor(delta_res).cuda(),
                    }

        elif stage == "test_for_stage1":
            if force_sga:
                compressed_z_mv = self.Quantizator_SGA(z_mv, it=iter, mode="training", total_it=total_iter)
            else:
                compressed_z_mv = self.quant(z_mv, mode)
            ...
            if force_sga:
                ...
                quant_mv = self.quant(mvfeature, mode, delta_mv)
            ...
            if force_sga:
                compressed_z = self.Quantizator_SGA(z, it=iter, mode="training", total_it=total_iter)
            else:
            ...
            if force_sga:
                compressed_y_renorm = self.Quantizator_SGA(feature_renorm, it=iter, mode="training", total_it=total_iter)
            else:
                compressed_y_renorm = self.quant(feature_renorm, mode, delta=1.0)
            ...
            total_bits_y, _, delta = self.feature_probs_based_sigma(
                feature_renorm, means_hat, scales_hat, mode, iter, total_iter, delta=delta_res, STEQuant=None, finetune=None)
            total_bits_mv, _, _ = self.feature_probs_based_sigma(
                mvfeature, means_hat_mv, scales_hat_mv, mode, iter, total_iter, delta=delta_mv, STEQuant=None, finetune=None)
            total_bits_z, _ = self.iclr18_estrate_bits_z(compressed_z)
            total_bits_z_mv, _ = self.iclr18_estrate_bits_z_mv(compressed_z_mv)
            ...
            compressed_y_renorm = self.quant(feature_renorm, mode, delta_res)
            ...
            total_bits_y, _, delta = self.feature_probs_based_sigma(
                feature_renorm, means_hat, scales_hat, mode, 0, total_iter, delta=delta_res, STEQuant=None, finetune=None)
            total_bits_mv, _, _ = self.feature_probs_based_sigma(
                mvfeature, means_hat_mv, scales_hat_mv, mode, 0, total_iter, delta=delta_mv, STEQuant=None, finetune=None)
            total_bits_z, _ = self.iclr18_estrate_bits_z(compressed_z)
            total_bits_z_mv, _ = self.iclr18_estrate_bits_z_mv(compressed_z_mv)
            ...
            total_bits_y, _ = self.feature_probs_based_sigma(
                feature_renorm, means_hat, scales_hat, mode, 0, STEQuant=None)
            total_bits_mv, _ = self.feature_probs_based_sigma(
                mvfeature, means_hat_mv, scales_hat_mv, mode, 0, STEQuant=None)
            total_bits_z, _ = self.iclr18_estrate_bits_z(compressed_z)
            total_bits_z_mv, _ = self.iclr18_estrate_bits_z_mv(compressed_z_mv)

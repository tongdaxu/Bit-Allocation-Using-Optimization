...
class DMC(CompressionModel):
    def __init__(self, anchor_num=4):
        ...
        # self.mv_y_q_scale = nn.Parameter(torch.ones((4, 1, 1, 1)))
        self.y_q_basic = nn.Parameter(torch.ones((1, channel_M, 1, 1)))
        # self.y_q_scale = nn.Parameter(torch.ones((4, 1, 1, 1)))
    ...
    def get_curr_mv_y_q(self, q_scale, q_basic):
        q_basic = LowerBound.apply(q_basic, 0.5)
        return q_basic * q_scale

    def get_curr_y_q(self, q_scale, q_basic):
        q_basic = LowerBound.apply(q_basic, 0.5)
        return q_basic * q_scale

    def compress(self, x, dpb, delta_mv=None, delta_res=None):
        # pic_width and pic_height may be different from x's size. x here is after padding
        # x_hat has the same size with x
        curr_mv_y_q = self.get_curr_mv_y_q(delta_mv, self.mv_y_q_basic)
        curr_y_q = self.get_curr_y_q(delta_res, self.y_q_basic)
    ...
    def decompress(self, dpb, string, height, width, delta_mv, delta_res):
        curr_mv_y_q = self.get_curr_mv_y_q(delta_mv, self.mv_y_q_basic)
        curr_y_q = self.get_curr_y_q(delta_res, self.y_q_basic)
        ...
        mv_y_hat_ = mv_y_hat * curr_mv_y_q

        mv_hat = self.mv_decoder(mv_y_hat_)
        context1, context2, context3, _ = self.motion_compensation(dpb, mv_hat)
        ...
        y_hat_ = y_hat * curr_y_q

        recon_image_feature = self.contextual_decoder(y_hat_, context2, context3)
    ...
    def encode_decode(self, x, dpb, output_path=None,
                      ...
                      delta_mv=None, delta_res=None):
        # pic_width and pic_height may be different from x's size. x here is after padding
        # x_hat has the same size with x
        if output_path is not None:
            delta_mv, mv_y_q_index = get_rounded_q(delta_mv)
            delta_res, y_q_index = get_rounded_q(delta_res)

            encoded = self.compress(x, dpb, delta_mv, delta_res)
    ...
    def compress_with_y(self, dpb, mv_y_=None, mv_z=None, y_=None, z=None, delta_mv=None, delta_res=None,
                        mv_y_q_basic=None, y_q_basic=None):
        # pic_width and pic_height may be different from x's size. x here is after padding
        # x_hat has the same size with x
        mv_y_q_basic = self.mv_y_q_basic if mv_y_q_basic is None else mv_y_q_basic
        y_q_basic = self.y_q_basic if y_q_basic is None else y_q_basic

        curr_mv_y_q = self.get_curr_mv_y_q(delta_mv, mv_y_q_basic)
        curr_y_q = self.get_curr_y_q(delta_res, y_q_basic)

        mv_y = mv_y_ / curr_mv_y_q
        mv_z_hat = torch.round(mv_z)
        mv_params = self.mv_hyper_prior_decoder(mv_z_hat)
        ref_mv_y = dpb["ref_mv_y"]
        if ref_mv_y is None:
    ...
    def encode_decode_with_y(self, dpb, mv_y_=None, mv_z=None, y_=None, z=None, delta_mv=None, delta_res=None,
                             mv_y_q_basic=None, y_q_basic=None, output_path=None, pic_width=None, pic_height=None):
        # pic_width and pic_height may be different from x's size. x here is after padding
        # x_hat has the same size with x
        if output_path is not None:
            delta_mv, mv_y_q_index = get_rounded_q(delta_mv)
            ...
            encoded = self.compress_with_y(dpb=dpb, mv_y_=mv_y_, mv_z=mv_z, y_=y_, z=z, delta_mv=delta_mv,
                                           delta_res=delta_res, mv_y_q_basic=mv_y_q_basic, y_q_basic=y_q_basic)
            encode_p(encoded['bit_stream'], mv_y_q_index, y_q_index, output_path)
            bits = filesize(output_path) * 8
            mv_y_q_index, y_q_index, string = decode_p(output_path)
            ...
            result = {
                ...
                "bit": bits,
                "decoding_time": decoding_time,
                "recon_image_enc": encoded["dbp"]["ref_frame"],
            }
            return result
    ...
    def forward(self, x, dpb, stage, mode, mv_y_=None, mv_z=None, y_=None, z=None, delta_mv=None, delta_res=None, it=0,
                total_it=0, mv_y_q_basic=None, y_q_basic=None, force_sga=False):
        if stage == "test_for_first":
            ...
            mv_y_ = self.mv_encoder(est_mv)
            mv_y = mv_y_ / curr_mv_y_q
            mv_z = self.mv_hyper_prior_encoder(mv_y)
            mv_z_hat = self.quant(mv_z, mode)
            mv_params = self.mv_hyper_prior_decoder(mv_z_hat)
            ref_mv_y = dpb["ref_mv_y"]
            if ref_mv_y is None:
                ref_mv_y = torch.zeros_like(mv_y)
            ...
            mv_y_res, mv_y_q, mv_y_hat, mv_scales_hat = self.forward_dual_prior(
                mv_y, mv_means, mv_scales, mv_q_step, self.mv_y_spatial_prior, mode, it, total_it, force_sga)
            mv_y_hat_ = mv_y_hat * curr_mv_y_q

            mv_hat = self.mv_decoder(mv_y_hat_)
            context1, context2, context3, warp_frame = self.motion_compensation(dpb, mv_hat)

            y_ = self.contextual_encoder(x, context1, context2, context3)
            y = y_ / curr_y_q
            z = self.contextual_hyper_prior_encoder(y)
            ...
            # warp_frame = torch.clamp(warp_frame, 0., 1.)
            ...
            bit = bpp * pixel_num
            bit_y = bpp_y * pixel_num
            bit_z = bpp_z * pixel_num
            bit_mv_y = bpp_mv_y * pixel_num
            bit_mv_z = bpp_mv_z * pixel_num
            ...
            mv_y = mv_y_ / curr_mv_y_q
            # mv_z = self.mv_hyper_prior_encoder(mv_y)
            mv_z_hat = self.SGA(mv_z, mode, it, total_it)
            mv_params = self.mv_hyper_prior_decoder(mv_z_hat)
            ref_mv_y = dpb["ref_mv_y"]
            if ref_mv_y is None:
                ref_mv_y = torch.zeros_like(mv_y)
            ...
            q_step, scales, means = self.y_prior_fusion(params).chunk(3, 1)
            y_res, y_q, y_hat, scales_hat = self.forward_dual_prior(
                y, means, scales, q_step, self.y_spatial_prior, mode, it, total_it, force_sga=False)
            y_hat_ = y_hat * curr_y_q
            ...
            mv_q_step, mv_scales, mv_means = self.mv_y_prior_fusion(mv_params).chunk(3, 1)
            mv_y_res, mv_y_q, mv_y_hat, mv_scales_hat = self.forward_dual_prior(
                mv_y, mv_means, mv_scales, mv_q_step, self.mv_y_spatial_prior, "test", it, total_it, force_sga=False)
            mv_y_hat_ = mv_y_hat * curr_mv_y_q
